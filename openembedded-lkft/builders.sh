...
( cd ${DEPLOY_DIR_IMAGE}
  [ -n "${BOOT_IMG}" ] && BOOT_IMG_SHA256SUM=$(sha256sum ${BOOT_IMG} | cut -d\  -f1)
  [ -n "${KERNEL_IMG}" ] && KERNEL_IMG_SHA256SUM=$(sha256sum ${KERNEL_IMG} | cut -d\  -f1)
  [ -n "${ROOTFS_IMG}" ] && ROOTFS_IMG_SHA256SUM=$(sha256sum ${ROOTFS_IMG} | cut -d\  -f1)
  [ -n "${ROOTFS_EXT4}" ] && ROOTFS_EXT4_SHA256SUM=$(sha256sum ${ROOTFS_EXT4} | cut -d\  -f1)
  [ -n "${ROOTFS_TARXZ_IMG}" ] && ROOTFS_TARXZ_IMG_SHA256SUM=$(sha256sum ${ROOTFS_TARXZ_IMG} | cut -d\  -f1)
  [ -n "${HDD_IMG}" ] && HDD_IMG_SHA256SUM=$(sha256sum ${HDD_IMG} | cut -d\  -f1)
)
...
BOOT_IMG_SHA256SUM=${BOOT_IMG_SHA256SUM}
KERNEL_IMG_SHA256SUM=${KERNEL_IMG_SHA256SUM}
ROOTFS_IMG_SHA256SUM=${ROOTFS_IMG_SHA256SUM}
ROOTFS_EXT4_SHA256SUM=${ROOTFS_EXT4_SHA256SUM}
ROOTFS_TARXZ_IMG_SHA256SUM=${ROOTFS_TARXZ_IMG_SHA256SUM}
HDD_IMG_SHA256SUM=${HDD_IMG_SHA256SUM}
EOF
